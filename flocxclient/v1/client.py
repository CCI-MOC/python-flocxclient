...
from flocxclient.common.http import DEFAULT_VER
...
class Client(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'os_flocx_api_version' not in kwargs:
            kwargs['os_flocx_api_version'] = DEFAULT_VER

        if not args and not kwargs.get('session'):
            raise TypeError("A session is required for creating a client, "
                            "use flocxclient.client.get_client to create "
                            "it automatically")
